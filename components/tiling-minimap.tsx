...
export function TilingMinimap({ pages, activePageIdx, onPageClick }: TilingMinimapProps) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-1.5 p-1.5 rounded-lg bg-background/80 backdrop-blur-md border border-border shadow-xl"
    >
      {pages.map((page, idx) => {
            ...
            <AnimatePresence>
              {hoveredIdx === idx && (
                <motion.div
                  ...
                  className="absolute right-full bottom-0 mr-2.5 w-52 p-2.5 rounded-md bg-popover backdrop-blur-md border border-border shadow-2xl pointer-events-none z-50"
                >
